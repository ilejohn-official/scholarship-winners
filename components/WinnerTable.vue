<template>
  <DataTable :value="winners" class="p-datatable-striped">
    <!-- Winner Photo -->
    <Column header="Photo">
      <template #body="{ data: { attributes } }">
        <Skeleton v-if="tableLoading" width="50px" height="50px" borderRadius="50%" />
        <Image v-else :src="attributes.winnerPhoto" alt="Winner Photo" width="50" height="50" borderRadius="50%"
          preview />
      </template>
    </Column>
    ...
    <Column field="attributes.winnerName" header="Winner Name">
      <template #body="{ data: { attributes } }">
        ...
        <span v-else>{{ attributes.winnerName }}</span>
      </template>
    </Column>
    ...
    <Column field="attributes.scholarshipTitle" header="Scholarship Title">
      <template #body="{ data: { attributes } }">
        ...
        <span v-else>{{ attributes.scholarshipTitle }}</span>
      </template>
    </Column>
    ...
    <Column field="attributes.amountWon" header="Amount Won">
      <template #body="{ data: { attributes } }">
        ...
        <span v-else>${{ attributes.amountWon.toLocaleString() }}</span>
      </template>
    </Column>
    ...
    <Column field="attributes.wonAt" header="Date Won">
      <template #body="{ data: { attributes } }">
        <Skeleton v-if="tableLoading" width="100px" />
        <span v-else>{{ formatDate(attributes.wonAt) }}</span>
      </template>
    </Column>
  </DataTable>
</template>
